                              ...
                              <div
                                dangerouslySetInnerHTML={{ 
                                  __html: formData.htmlContent
                                    ...
                                    .replace(/{{cohortName}}/g, 'Cohort #1')
                                    .replace(/{{cohortDescription}}/g, 'Sample cohort description')
                                    .replace(/{{questionTitle}}/g, 'Sample Question Title')
                                    .replace(/{{questionNumber}}/g, '1')
                                    .replace(/{{questionUrl}}/g, 'https://example.com/question/1')
                                    .replace(/{{dashboardUrl}}/g, 'https://example.com/dashboard')
                                    .replace(/{{grade}}/g, 'Approved')
                                    .replace(/{{gradePoints}}/g, '95')
                                    .replace(/{{feedback}}/g, 'Great work!')
                                    .replace(/{{submissionTime}}/g, new Date().toLocaleString())
                                    .replace(/{{resetUrl}}/g, 'https://example.com/reset-password')
                                    .replace(/{{miniQuestionTitle}}/g, 'Sample Mini Question')
                                    .replace(/{{contentTitle}}/g, 'Sample Content Title')
